...
- systémový disk: 400GB SATA SSD;
- datové disky: 4× 8TB SAS Seagate Exos 7E8;
...
Čtyři 8TB SAS disky nejsou jedním čtyřdiskovým poolem. Jsou rozdělené takto:

```text
tank-pbs → mirror ze 2× 8TB SAS
tank-nas → mirror ze 2× 8TB SAS
```
...
Autoritativní dokumentace zálohování je v [PBS a disaster recovery](../Zalohy/PBS-DR.md).
...
VM400, VM401, VM402 a VM501 jsou záměrně ponechané, dokud nebude jejich další osud jednotlivě rozhodnutý. Nelze je souhrnně označit za nepotřebné.
...
- [ ] Ověřit aktuální stav, obsazení a poslední scrub poolů `tank-pbs` a `tank-nas`.
- [ ] Ověřit SMART a teploty čtyř SAS disků a systémového SSD.
- [ ] Prakticky ověřit start celého Dellu, VM200 a datastore po úplném výpadku napájení.
